...
| UX descripció Roger (text usuari vs plantilla) | **No en el repo actual** — ver [Pendiente](#pendiente-y-discutido) |
...
### 2. UX descripció para Roger (texto del usuario vs plantilla Jotun/Isaval)

**Problema**: al añadir partida, `generateBudgetDraft` rellena `description` con la **plantilla técnica**, no con el texto que escribe Roger en el textarea de IA.

**Solución diseñada en sesión** (no presente en el árbol git actual):

- `description` = texto del cliente (Roger); `suggestedDescription` = plantilla (solo UI).
- `BudgetLine.clientDescription` desde la IA.
- `mergeDraftClientItems`, `resolveDraftItemDescription`, `enrichParsedLines`.
- Componente `BudgetItemDescriptionFields` (descripció + suggerència + «Aplicar suggerència»).
- Merge síncrono en `handleSubmit` para evitar condición de carrera con `useEffect`.

**Estado**: implementado y probado en sesión de agente (166 tests en ese momento), pero **los archivos no están en el repo** (nunca commiteados; el working tree actual no los contiene). Reimplementar o recuperar desde backup si se quiere cerrar esta UX.
...
- **Descripciones al crear**: siguen siendo las **plantillas por tipo** (`generateBudgetDraft`); Roger debe editarlas manualmente en el textarea de cada tarjeta hasta implementar el punto 2 de pendientes.
